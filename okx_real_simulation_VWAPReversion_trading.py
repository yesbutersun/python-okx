...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('okx_real_simulation_VWAPReversion_trading.log', encoding='utf-8'),
...
class OKXRealSimulationTrader:
    ...
    def __init__(
        ...
        trading_config_file='strategies/vwap_reversion.json',
        ...
        # 策略参数
        # 不要覆盖 load_config/reset_trading_state 中的配置值
        self.lookback = getattr(self, 'lookback', 20)
        self.std_dev = getattr(self, 'std_dev', 2.0)

        # 杠杆设置
        self.leverage = getattr(self, 'leverage', 5)
    ...
    def reset_trading_state(self):
        ...
        self._last_server_date = None
        self._last_server_time_fetch = 0
        # keep leverage from config; only fallback when missing
        self.leverage = getattr(self, 'leverage', 5)

        logger.info("Trading state reset done")
    ...
    def generate_signals(self, df):
        ...
        try:
            ...
            if strategy_name == 'VWAPReversion':
                required_len = max(required_len, int(strategy_kwargs.get('vwap_len', getattr(self, 'vwap_len', 30))))

            if len(df) < required_len:
                logger.warning(f"⚠️ 数据不足，需要至少{required_len}条，当前{len(df)}条")
                return None

            signals = run_strategy(df, strategy_name, **strategy_kwargs)
            ...
            logger.info(f"🧠 策略: {strategy_name} | params={strategy_kwargs}")
...
def main(sandbox=True, trading_duration=30, trading_config_file=None, symbol=None):
    ...
    try:
        trader = OKXRealSimulationTrader(
            trading_config_file=trading_config_file or 'strategies/vwap_reversion.json',
...
if __name__ == "__main__":
    ...
    try:
        parser = argparse.ArgumentParser(description="OKX VWAP Reversion Trading")
        ...
        parser.add_argument('--duration', type=int, default=7200, help='交易时长（分钟）')
        parser.add_argument('--config', default='strategies/vwap_reversion.json', help='策略配置文件路径')
